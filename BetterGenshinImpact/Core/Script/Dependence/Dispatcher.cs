...
public class Dispatcher
{
    ...
    {  
        ...
        CancellationToken cancellationToken = customCt ?? CancellationContext.Instance.Cts.Token;
        var fightSoloTask = CombatTaskFactoryProvider.GetFactory(param.CombatStrategyPath).CreateTask(param);
        await fightSoloTask.Start(cancellationToken);  
    }
    ...
    /// <summary>
    /// 执行简易战斗策略（JS 接口）。
    /// 支持逗号、换行、分号分隔的简易策略字符串，如 "e,重击,等待(2)"。
    /// </summary>
    /// <param name="script">简易策略字符串</param>
    /// <param name="avatarName">可选：指定执行角色名，省略则使用当前场上角色</param>
    /// <param name="customCt">自定义取消令牌</param>
    public async Task RunCombatScript(string script, string? avatarName = null, CancellationToken? customCt = null)
    {
        ...
        if (combatScript.CombatCommands.Count == 0)
        {
            return;
        }
        ...
        // 4. 依次执行策略指令（传入 lastCommand 以优化同角色切换）
