const ROWS_PER_SCREEN = 5;   // 屏幕显示的行数
const WEAPON_POSITIONS_FILE = "weapon_positions.json";  // 武器位置记录文件（保存到脚本同目录）
...
(async function () {
    ...
    // 加载武器位置记录
    const weaponPositions = await loadWeaponPositions(WEAPON_POSITIONS_FILE);
    ...
    // 加载武器位置记录
    async function loadWeaponPositions(filePath) {
        try {
            const jsonData = file.readTextSync(filePath);
            return JSON.parse(jsonData);
        } catch (error) {
            log.info("未找到武器位置记录文件，将创建新文件");
            return {};
        }
    }

    // 记录武器位置到文件
    function recordWeaponPosition(weaponName, gridPos) {
        weaponPositions[weaponName] = gridPos;
        log.info(`已记录武器位置: ${weaponName} → 行 ${Math.floor(gridPos / 10)} 列 ${gridPos % 10}`);
        try {
            file.writeTextSync(WEAPON_POSITIONS_FILE, JSON.stringify(weaponPositions, null, 2));
        } catch (error) {
            log.error(`保存武器位置记录失败: ${error}`);
        }
    }
  ...
  // 识别并组合武器名称
    ...
    for (let scroll = 0; scroll <= pageScrollCount; scroll++) {
        ...
        for (let row = 0; row < maxRows; row++) {
            for (let column = 0; column < maxColumns; column++) {
                ...
                if (matchRatio >= 0.8) { // 如果匹配占比大于等于 80%，则认为匹配成功
                    ...
                    // 记录武器位置供后续快速模式使用
                    const absoluteRow = scroll * maxRows + row + 1;
                    const recordedGridPos = absoluteRow * 10 + (column + 1);
                    recordWeaponPosition(settingsWeapon, recordedGridPos);
                    await click(1600, 1005); // 点击确认
                    await sleep(1000);
                    await click(1320, 755); // 点击确认
                    await sleep(1000);
        ...
        if (scroll < pageScrollCount) {
            await scrollPage(ROWS_PER_SCREEN * rowHeight, 10, 10); // 按行数滚动页面
        }
    }
...
async function scanWeaponsQuick(settingsWeapon, storedGridPos = null) {
    log.info("【快速模式】开始");

    // 解析 gridPosition：优先使用手动设置，其次使用已记录的位置
    let gridPos = settings.gridPosition || storedGridPos;
    if (!gridPos) {
        log.warn("快速模式下未设置 gridPosition，回退到普通模式");
        return false;
    }
    const posStr = String(gridPos);
    let row, col;
    if (posStr.length === 3) {
        row = parseInt(posStr.substring(0, 2), 10);
        col = parseInt(posStr.charAt(2), 10);
    } else if (posStr.length === 2) {
        row = parseInt(posStr.charAt(0), 10);
        col = parseInt(posStr.charAt(1), 10);
    } else {
        log.warn(`快速模式下无效的 gridPosition 长度: ${posStr.length}，回退到普通模式`);
        return false;
    }
    if (isNaN(row) || isNaN(col) || row < 1 || row > 99 || col < 1 || col > 4) {
        log.warn(`快速模式下无效的 gridPosition: ${gridPos}，回退到普通模式`);
        return false;
    }
    ...
    // 如果行数超过屏幕显示行数(5)，需要分批滚动，每次最多5行
    if (row > ROWS_PER_SCREEN) {
        let remainingRows = row - ROWS_PER_SCREEN;
        // 每次滚动最多5行，直到剩余行数为0
        while (remainingRows > 0) {
            const batchRows = Math.min(remainingRows, ROWS_PER_SCREEN);
            const scrollDistance = batchRows * rowHeight;
            log.info(`快速模式：向下滚动 ${batchRows} 行（${scrollDistance} 像素），剩余 ${remainingRows - batchRows} 行`);
            await scrollPage(scrollDistance, 10, 10);
            await sleep(300);
            remainingRows -= batchRows;
        }
        // 滚动完成后，目标行现在位于屏幕的第5行位置
        row = ROWS_PER_SCREEN;
    }

    // 计算点击的格子坐标（行：row-1，列：col-1）
    const targetRow = row - 1;
    const targetCol = col - 1;
    const clickX = Math.round(startX + targetCol * columnWidth);
    const clickY = Math.round(startY + targetRow * rowHeight);
    log.info(`快速模式：点击第 ${row} 行第 ${col} 列，坐标(${clickX}, ${clickY})`);

    await click(clickX, clickY);
    await sleep(200);

    // 识别当前选中的武器名称
    const recognizedWeapon = await recognizeAndCombineWeaponName(ocrRegion);
    if (!recognizedWeapon) {
        log.warn("快速模式：未识别到武器名称，回退到普通模式");
        return false;
    }

    // 获取目标武器的正式名称
    const targetInfo = await getTargetWeaponName(settingsWeapon);
    if (!targetInfo) {
        log.warn("快速模式：无法获取目标武器信息，回退到普通模式");
        return false;
    }
    const { weaponName1, weaponNames } = targetInfo;

    let matchedWeapon = fuzzyMatch(recognizedWeapon, weaponNames, 1);
    if (!matchedWeapon) matchedWeapon = recognizedWeapon;
    const matchRatio = calculateMatchRatio(weaponName1, matchedWeapon);

    if (matchRatio >= 0.8) {
        log.info(`✅ 快速模式找到目标武器（${matchedWeapon}），匹配度 ${matchRatio.toFixed(2)}`);
        recordWeaponPosition(settingsWeapon, gridPos);
        await click(1600, 1005);
        await sleep(1000);
        await click(1320, 755);
        await sleep(1000);
        return true;   // 直接返回成功，不再做任何额外验证
    } else {
        log.warn(`快速模式：武器不匹配（${recognizedWeapon} vs ${weaponName1}），回退到普通模式`);
        return false;
    }
}

// 辅助函数 getTargetWeaponName（保持不变）
async function getTargetWeaponName(settingsWeapon) {
    const characterName = aliasToNameMap[Character] || Character;
    if (!characterName) return null;
    const characterWeaponType = nameToWeaponMap[characterName];
    const weaponType = weaponTypeMap[characterWeaponType];
    let weaponNames = weaponNamesMap[weaponType] || [];
    if (!weaponNames.length) weaponNames.push(settingsWeapon);
    let weaponName1 = fuzzyMatch(settingsWeapon, weaponNames, 0.9);
    if (!weaponName1) weaponName1 = settingsWeapon;
...
// 执行角色路径
    async function CharacterPath() {
        ...
        await sleep(1000);
        // 未手动指定行列位置时，检查当前装备武器是否已是目标武器
        if (!settings.gridPosition) {
            const currentWeapon = await recognizeAndCombineWeaponName(ocrRegion);
            if (currentWeapon) {
                const targetInfo = await getTargetWeaponName(Weapon);
                if (targetInfo) {
                    let matched = fuzzyMatch(currentWeapon, targetInfo.weaponNames, 1);
                    if (!matched) matched = currentWeapon;
                    if (calculateMatchRatio(targetInfo.weaponName1, matched) >= 0.8) {
                        log.info(`当前装备武器(${matched})已匹配目标(${Weapon})，无需更换`);
                        await genshin.returnMainUi();
                        return;
                    }
                }
        ...
        // 优先级：手动设置的 gridPosition > 已记录位置 > 普通模式
        const manualGridPos = settings.gridPosition;
        const storedGridPos = weaponPositions[Weapon] ?? null;
        const effectiveGridPos = manualGridPos || storedGridPos;

        if (settings.quickMode && effectiveGridPos) {
            if (manualGridPos) {
                log.info(`使用手动设置的武器位置: ${Weapon} → 行 ${Math.floor(manualGridPos / 10)} 列 ${manualGridPos % 10}`);
            } else {
                log.info(`使用已记录的武器位置: ${Weapon} → 行 ${Math.floor(storedGridPos / 10)} 列 ${storedGridPos % 10}`);
            }
            weaponFound = await scanWeaponsQuick(Weapon, effectiveGridPos);
            if (!weaponFound) {
                if (manualGridPos) {
                    log.error(`手动指定的位置(${manualGridPos})未找到 ${Weapon}，请重新设置行列位置`);
                }
                log.info("快速模式失败，回退到普通扫描模式，重置武器滑条");
                ...
                leftButtonUp();
                await sleep(200);
                weaponFound = await scanWeapons(Weapon);
            }
        } else {
            if (!settings.quickMode) {
                log.info("快速模式已关闭，直接运行普通扫描模式");
            } else {
                log.info("未找到武器位置记录，直接运行普通模式");
            }
            weaponFound = await scanWeapons(Weapon);
        }
