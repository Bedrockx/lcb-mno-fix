...
public partial class TaskSettingsPageViewModel : ViewModel
{
    ...
    /// <summary>
    /// 联机锄地是否可见（隐藏功能，点击独立任务标题10次解锁/锁定）
    /// </summary>
    [ObservableProperty]
    ...
    // /// <summary>
    // /// 独立任务标题点击计数，达到10次切换联机锄地的显示/隐藏状态
    // /// </summary>
    // [RelayCommand]
    ...
    //         Wpf.Ui.Violeta.Controls.Toast.Success(newState ? "联机锄地已解锁" : "联机锄地已锁定");
    ...
    public async Task OnSwitchAutoFight()
    {
        ...
        await new TaskRunner()
            .RunSoloTaskAsync(fightSoloTask);
        SwitchAutoFightEnabled = false;
    }
    ...
    public bool GetFightStrategy(string strategyName, out string path)
    {
        ...
        path = AutoFightParam.ResolveStrategyPath(strategyName);
