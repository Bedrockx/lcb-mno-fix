...
public partial class CommonSettingsPageViewModel : ViewModel
{
    ...
    [ObservableProperty] private List<string> _oneDragonFlowNames = [];
    [ObservableProperty] private List<string> _scriptGroupNames = [];

    private void LoadIdleActionLists()
    {
        // 加载一条龙配置列表
        var oneDragonFolder = Global.Absolute(@"User\OneDragon");
        if (Directory.Exists(oneDragonFolder))
        {
            OneDragonFlowNames = Directory.GetFiles(oneDragonFolder, "*.json")
        ...
        // 加载配置组列表
        var groupFolder = Global.Absolute(@"User\ScriptGroup");
        if (Directory.Exists(groupFolder))
        {
            ScriptGroupNames = Directory.GetFiles(groupFolder, "*.json")
                .Select(f => Path.GetFileNameWithoutExtension(f))
                .OrderBy(n => n)
                .ToList();
        }
