...
public class TaskRunner
{
    ...
    public async Task RunCurrentAsync(Func<Task> action, bool resetCancellationContext = true, bool clearCancellationContextOnLockFailure = false)
    {
        ...
        finally
        {
            ...
            // 在 Clear() 之前捕获 token，避免 Clear() 释放 Cts 后访问已释放对象
            var idleCt = CancellationContext.Instance.Cts.Token;
            ...
            // 空闲超时检测：信号量释放后，如果没有新任务启动，执行预设的空闲动作
            if (hasLock && TaskContext.Instance().Config.OtherConfig.IdleActionConfig.Enabled)
            {
                _ = IdleActionExecutor.ExecuteIfIdleAsync(TaskContext.Instance().Config, idleCt);
            }
        }
    }
