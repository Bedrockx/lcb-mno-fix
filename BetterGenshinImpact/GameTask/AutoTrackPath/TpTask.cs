...
public class TpTask
{
    ...
    {
        // 失败后重试最多25次（每次间隔200ms），覆盖地图刚打开时缩放按钮未渲染完成的场景
        // 每次重试重新截图（region 是传入时的快照，不更新永远看不到缩放按钮）
        double s = 0;
        for (int i = 0; i < 25; i++)
        {
            try
            {
                using var currentRegion = CaptureToRectArea();
                s = Bv.GetBigMapScale(currentRegion);
                if (s > 0) break;
            }
            catch
            {
            ...
            if (i < 24)
            {
                TaskControl.Logger.LogWarning("获取大地图缩放级别失败，重试中...（{Attempt}/25）", i + 1);
                Thread.Sleep(200);
            }
        }
